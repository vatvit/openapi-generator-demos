<?php

declare(strict_types=1);

/**
 * Tic Tac Toe
 * 
 * This API allows writing down marks on a Tic Tac Toe board and requesting the state of the board or of individual squares.
 * 
 * PHP version 
 *
 * @category Class
 * @package  TicTacToeApiV2\Server\Models
 * @author   OpenAPI Generator
 * @link     https://openapi-generator.tech
 */

/**
 * NOTE: This class is auto-generated by OpenAPI Generator
 * https://openapi-generator.tech
 * Do not edit this class manually.
 */

namespace TicTacToeApiV2\Server\Models;

/**
 * CreateGameRequest DTO
 *
 * Data Transfer Object for CreateGameRequest
 * Auto-generated from OpenAPI schema
 *
 * @category DTO
 * @package  TicTacToeApiV2\Server\Models
 * @author   OpenAPI Generator
 * @link     https://openapi-generator.tech
 */
class CreateGameRequest
{
    /**
     * Constructor
     *
     * @param \TicTacToeApiV2\Server\Models\GameMode $mode
     * @param string $opponentId Opponent player ID (required for PvP mode)
     * @param bool $isPrivate Whether the game is private
     * @param array<string,mixed> $metadata Additional game metadata
     */
    public function __construct(
        public \TicTacToeApiV2\Server\Models\GameMode $mode,
        public string $opponentId,
        public bool $isPrivate = false,
        public array<string,mixed> $metadata,
    ) {}

    /**
     * Create from array
     *
     * @param array<string, mixed> $data
     * @return self
     */
    public static function fromArray(array $data): self
    {
        return new self(
            mode: $data['mode'],
            opponentId: $data['opponentId'] ?? null,
            isPrivate: $data['isPrivate'] ?? null,
            metadata: $data['metadata'] ?? null,
        );
    }

    /**
     * Convert to array
     *
     * @return array<string, mixed>
     */
    public function toArray(): array
    {
        return [
            'mode' => $this->mode,
            'opponentId' => $this->opponentId,
            'isPrivate' => $this->isPrivate,
            'metadata' => $this->metadata,
        ];
    }
}
